...
export default function PricingPage() {
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-primary-50/30 relative overflow-hidden">
			...
			<div
				className="absolute bottom-40 -right-40 w-96 h-96 bg-purple-200/20 rounded-full blur-3xl animate-float"
				...
				<div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
					<Link to="/" className="flex items-center gap-3">
						<span className="text-xl font-bold text-gray-900">
							Hire<span className="text-primary-600">Glint</span>
							<span className="text-primary-500 text-sm">.ai</span>
						</span>
					</Link>
			...
			<div className="relative z-10 max-w-7xl mx-auto px-6 pt-16 pb-12 text-center">
				...
				<p
					className="mt-4 text-lg text-gray-500 max-w-2xl mx-auto animate-fade-in-up"
				...
				>
					...
					<div className="flex items-center gap-3">
						<span
							className={`text-sm font-medium ${!isStudent ? "text-gray-900" : "text-gray-400"}`}
						>
							Business
						</span>
						<button
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
						{tiers.map((tier, index) => (
							...
							>
								...
								<div className="mb-6">
									<h3
										className={`text-xl font-bold ${tier.highlight ? "text-white" : "text-gray-900"}`}
									>
										{tier.name}
									</h3>
									<p
										className={`mt-2 text-sm ${tier.highlight ? "text-primary-100" : "text-gray-500"}`}
									>
										{tier.description}
									</p>
								</div>
								...
								<div className="mb-6">
									<div className="flex items-baseline gap-1">
										...
										<span
											className={`text-sm ${tier.highlight ? "text-primary-200" : "text-gray-400"}`}
										>
											/mo
										</span>
									</div>
									{annual && (
										<p
											className={`text-xs mt-1 ${tier.highlight ? "text-primary-200" : "text-gray-400"}`}
										>
											Billed annually (
											{formatPrice(
								...
								<ul className="mt-8 space-y-3">
									{tier.features.map((feature) => (
										<li key={feature} className="flex items-start gap-3">
											...
											<span
												className={`text-sm ${tier.highlight ? "text-primary-50" : "text-gray-600"}`}
											>
												{feature}
											</span>
										</li>
